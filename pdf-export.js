...
const PDFModule = (() => {
  ...
  async function buildDoc(vehicleId) {
    ...
    // Patente todo a la derecha dentro del recuadro
    if (v.patente) {
      doc.setFontSize(10); doc.setFont('helvetica','bold');
      const pw = doc.getTextWidth(v.patente) + 10;
      const px = pageW - margin - 5 - pw;
  ...
  async function generatePDFWithPhotos(vehicleId, silent) {
    ...
    try {
      ...
      if (images.length > 0 || firmaImg) {
        ...
        if (images.length > 0) {
          const cols   = 4;
          const gapF   = 2.6; // ~10px en todos los lados
          const marginF = 3;
