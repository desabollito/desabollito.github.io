...
function openVehicleDetail(id) {
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    ${estadosHtml}
    ${piezasActivas.length ? `<div class="detail-section" style="margin-top:14px"><h4>Piezas Afectadas</h4><div class="pieces-chips">${piezasActivas.map(p=>`<span class="chip">${p}</span>`).join('')}</div></div>` : ''}
